...
import { createFabricProperties } from "../graphql/mutations";
export default function FabricPropertiesCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setFabricName(initialValues.fabricName);
    setElasticity(initialValues.elasticity);
    setBounce(initialValues.bounce);
    setErrors({});
  };
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: createFabricProperties.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "FabricPropertiesCreateForm")}
      {...rest}
    >
      <TextField
        label="Fabric name"
